const Song = ({ track, large, hover }) => {
  return (
    <>
      <div className={"sc-flex sc-mr-2 sc-bg-white sc-p-2" + (hover ? ' sc-hover:bg-gray-200 sc-hover:font-bold' : '')}>
        {/* Image on left */}
        <div className="sc-w-1/4 sc-h-max">
          <img
            ...
            className={"sc-h-[10em] sc-rounded sc-w-full sc-object-cover" + (track.link ? " sc-cursor-pointer" : "") + (!large ? " sc-h-[5em]" : "")}
          />
        </div>

        {/* Song info on right */}
        <div className={"sc-w-3/4" + (large ? " sc-flex sc-flex-col sc-justify-center" : "")}>
          <div className="sc-flex sc-flex-col sc-w-100">
            {/* Title */}
            <div className={"sc-flex sc-items-start sc-flex-col sc-py-3 sc-w-100" + (large && " sc-px-5")}>
              <div className="sc-flex sc-flex-row sc-mb-4">
                ...
                <div>
                  <h5 className="sc-text-gray-400 sc-text-sm">
                    {track.artist ? (track.artist) : ('James Hinton')}</h5>
                  <h4 className={"sc-text-gray-800" + (large ? " sc-text-xl" : " sc-text-sm" )}>{track.title}</h4>
                </div>
              </div>
